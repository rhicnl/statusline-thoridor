...
| `install` | Copy the extension into the scope's Pi extensions dir (also clears a stale disable); always writes the scope's `thoridor.json` config — from `--profile`/`--glyphs`, else previously configured values, else defaults `magni`+`nerd` — and creates the scope's `settings.json` if missing (existing content is never touched) |
